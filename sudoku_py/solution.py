...
class solve():
    ...
    def backpropagation(self, set_prime):
        # Convert to Array
        ...
        while i < len(set_prime):
            while j < len(set_prime):
                if set_sol[i, j] == 0:
                    ...
                    status = True
                    while status:
                        possible = self.check((i, j), set_sol)
...
if __name__ == '__main__':
    ...
    content = [
        ...
        [0, 0, 0, 1, 0, 0, 9, 7, 0]]
    answer = solve()
    #solve.backpropagation(sample)
    abc = answer.backpropagation(sample)
    print(abc)
